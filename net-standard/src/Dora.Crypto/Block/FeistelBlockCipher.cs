...
public class FeistelBlockCipher : IBlockCipher
{
    ...
    public FeistelBlockCipher(IKeySchedule keySchedule, IRoundFunction roundFunction, int blockSize)
    {
    ...
    public byte[] Encrypt(byte[] plaintext)
    {
        ...
        if (plaintext.Length != _blockSize) throw new ArgumentException("Invalid block size", nameof(plaintext));

        // (1) Split the block into two equal parts.
        byte[] l = new byte[plaintext.Length / 2];
        byte[] r = new byte[plaintext.Length / 2];
        Array.Copy(plaintext, 0, l, 0, l.Length);
        Array.Copy(plaintext, plaintext.Length / 2, r, 0, r.Length);
    ...
    public byte[] Decrypt(byte[] ciphertext)
    {
        ...
        if (_roundKeys == null) throw new InvalidOperationException("Cipher is not initialized");
        if (ciphertext.Length != _blockSize) throw new ArgumentException("Invalid block size", nameof(ciphertext));

        // (1) Split the block into two equal parts.
        byte[] r = new byte[ciphertext.Length / 2];
        byte[] l = new byte[ciphertext.Length / 2];
        Array.Copy(ciphertext, 0, r, 0, r.Length);
        Array.Copy(ciphertext, ciphertext.Length / 2, l, 0, l.Length);
